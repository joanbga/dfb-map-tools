...
- **Missing arguments** : Displays correct command usage
- **Invalid binary file** : Map file reading failure
- **Invalid cell IDs** : Non-numeric values or out-of-range values
- **Non-existent cells** : Attempt to access a cell that doesn't exist on the map
- **DFB exceptions** : Domain-specific errors caught and displayed

## Project Structure

The program uses several components:

- `MapReader` : Reading binary map files
- `Map` : Map representation and manipulation
- `MapCell` : Individual cell representation
- `Commands` : Command management system
- `DfbException` : Specific exception handling

## File Formats

The program reads map files in binary format. The exact format depends on the `MapReader::readMapFromBinary()` implementation.
...
# Check if cell 100 has four free neighbors
./map_utils map.bin hasFourAdjacentCellsFree 100

# Check with cells marked as occupied
./map_utils map.bin hasFourAdjacentCellsFree 100 50 75 120 200

# Get neighbors of cell 100
./map_utils map.bin getNeighbors 100
```
...
- `0` : Success
- `1` : Error (invalid arguments, file not found, non-existent cell, etc.)

## Dependencies

- C++ compiler compatible with C++11 or higher
- Custom libraries: MapReader, Map, MapCell, Commands
- C++ standard library (iostream, vector, set)